...
def getRequest(species):
	"""Get the request for a species."""
	# Sanitize all values of the species.
	modifiedSpecies = {k: escapeValue(v) for k, v in species.items()}
	return "INSERT INTO species VALUES ('{name}', '{taxon}', '{order}', '{family}', '{genus}', '{species}', {extinct}, '{commonNames}', '{breedingRegion}', '{breedingSubregion}', '{nonbreedingSubregion}')".format(**modifiedSpecies)

def getRequests(species):
	"""Get the requests for all the species."""
	requests = []
	for s in species:
...
def saveRequests(species):
	"""Save the requests for all the species."""
	file = open("species.sql", "w", encoding="utf8")
	requests = getRequests(species)
